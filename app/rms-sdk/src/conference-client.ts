...
export type StartOptions = {
  ...
  createMicrophoneStream?: (params: { microphoneDeviceId?: string | null }) => Promise<MediaStream>
}
...
export class ConferenceClient {
  ...
  async start(options: StartOptions) {
    try {
      ...
      await this.signaling.connect(resolveWsUrl(options))
  ...
  async setMicEnabled(enabled: boolean) {
    try {
      ...
      if (!this.localAudioTrack) {
        const rawStream =
          (await this.startOptions?.createMicrophoneStream?.({
            microphoneDeviceId: this.startOptions.microphoneDeviceId
          })) ??
        ...
        this.localAudioTrack = rawStream.getAudioTracks()[0] ?? null
...
function resolveWsUrl(options: StartOptions): string {
  if (options.joinToken && options.rmsUrl) {
    const url = new URL('/v1/connect', options.rmsUrl)
    url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:'
    url.searchParams.set('token', options.joinToken)
    return url.toString()
  }
