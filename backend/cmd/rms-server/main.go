...
func main() {
	...
	router := media.NewSFU(api, hub, lookup)
	coordinator := application.NewSignalingCoordinator(roomRepo, sessionRepo, hub, lookup, &mediaBridgeAdapter{router: router})
	tokens := application.NewJoinTokenService(cfg.RMSJoinSecret, "kvatum-rms", clock, 10*time.Minute)
	server := httpadapter.NewRealtimeMediaServer(roomService, coordinator, hub, tokens, cfg.RMSAdminAPIKey, cfg.RMSPublicURL)

	log.Printf("rms listening on %s", cfg.HTTPAddr)
...
func (a *mediaBridgeAdapter) HandleSubscriberAnswer(participantID string, answer webrtc.SessionDescription) (*webrtc.SessionDescription, error) {
	return a.router.HandleSubscriberAnswer(participantID, answer)
}

func (a *mediaBridgeAdapter) HandleSubscriberCandidate(participantID string, candidate webrtc.ICECandidateInit) error {
	return a.router.HandleSubscriberCandidate(participantID, candidate)
}
