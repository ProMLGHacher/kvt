  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {participants.map((participant) => {
        ...
        const awaitingMedia =
          !stream && (audioSlot?.enabled || cameraSlot?.enabled || screenSlot?.enabled)
          ...
          >
                ...
                <div className="text-center text-sm text-slate-300">
                  ...
                  <p>
                    {screenSlot?.enabled
                      ? 'Screen share ready'
                      : cameraSlot?.enabled
                        ? 'Camera signal is on, waiting for media…'
                        : 'Camera is off'}
                  </p>
                  {awaitingMedia ? (
                    <p className="mt-2 text-xs text-slate-400">
                      Participant is present, but remote media has not attached yet.
                    </p>
                  ) : null}
                </div>
              )}
            ...
            <div className="flex items-center justify-between gap-3 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {screenSlot?.enabled ? (
                  <Badge className="bg-accent text-accent-foreground">Screen</Badge>
                ) : null}
                {awaitingMedia ? (
                  <Badge className="bg-amber-100 text-amber-900">Waiting for media</Badge>
                ) : null}
              </div>
            </div>
          </article>
...
const ParticipantVideo = memo(function ParticipantVideo({
  ...
  useEffect(() => {
    ...
    const handlePlaying = () => {
      logInfo('room-media', 'video playing', {
        participantId,
        isLocal,
    ...
    const handleWaiting = () => {
      logInfo('room-media', 'video waiting', {
        participantId,
        isLocal,
        readyState: node.readyState
      })
    ...
    const handleStalled = () => {
      logInfo('room-media', 'video stalled', {
        participantId,
        isLocal,
        readyState: node.readyState
      })
  ...
  return (
    <video autoPlay muted={muted} playsInline ref={ref} className="h-full w-full object-cover" />
  )
})
